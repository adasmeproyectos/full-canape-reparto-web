...
// Importamos Phone y MessageCircle para los nuevos botones dinámicos
...
function App() {
  ...
  const handleProcesar = async () => {
    ...
    try {
      ...
      // Estructuramos los datos para añadir los estados de check
  ...
  // Funciones de WhatsApp Directo dinámicas
  ...
  const BotonRayo = () => (
    <button onPointerDown={() => {
      ...
      className="relative overflow-hidden bg-stone-100 w-20 h-20 rounded-2xl flex flex-col items-center justify-center border-2 border-stone-200 active:scale-90 transition-transform select-none touch-none shrink-0 shadow-inner">
  ...
  if (pedidos.length === 0) {
    return (
      <div className="min-h-screen bg-stone-50 p-6 flex flex-col justify-center max-w-xl mx-auto">
        ...
        <textarea className="w-full h-64 p-5 bg-white rounded-3xl border-2 mb-8 outline-none text-base resize-none focus:border-red-400 transition-colors shadow-inner" placeholder="Pega los pedidos aquí..." value={input} onChange={(e) => setInput(e.target.value)} />
  ...
  return (
    <div className="min-h-screen bg-stone-100 p-4 sm:p-6 flex flex-col relative overflow-hidden max-w-7xl mx-auto">
      ...
      {/* Selector de modo (Check-in / Ruta) */}
      <div className="bg-stone-200/70 p-1.5 rounded-2xl flex mb-6 shadow-inner shrink-0">
      ...
      <div className="flex-1 bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-xl border-t-8 border-red-600 flex flex-col relative overflow-hidden">
        
        {/* Cabecera compartida (Número de parada) */}
        ...
        {modo === 'reparto' ? (
          // 🔥 MODO HORIZONTAL OPTIMIZADO PARA REPARTO 🔥
          <div className="flex flex-col md:flex-row flex-1 gap-6 md:gap-8 overflow-hidden">
            
            {/* COLUMNA IZQUIERDA (FIJA EN HORIZONTAL): Datos del cliente y acciones */}
            <div className="w-full md:w-2/5 flex flex-col gap-4 md:gap-5 shrink-0 overflow-y-auto no-scrollbar md:pb-4">
              ...
              <p className="text-sm font-bold text-stone-600 flex items-start bg-stone-100 p-4 rounded-xl shadow-inner"><MapPin size={16} className="mr-2 mt-0.5 text-red-500 shrink-0"/> <span className="leading-snug">{pedidoActual.direccion}</span></p>
              
              <button onClick={() => abrirMapa(pedidoActual.direccion)} className="w-full bg-stone-800 text-white p-4 rounded-2xl font-black flex justify-center items-center active:scale-95 transition-all shadow-md shrink-0"><Map size={20} className="mr-2" /> NAVEGAR CON MAPS</button>
              
              {/* PANEL DE COMUNICACIÓN (Avisos de WhatsApp) */}
              <div className="bg-stone-50 p-4 rounded-3xl border-2 border-stone-100 shrink-0">
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  {pedidoActual.telefono ? (
                    <button onClick={enviarAvisoCliente} disabled={!eta} className="flex-1 bg-[#25D366] hover:bg-[#20bd5a] text-white font-black p-4 rounded-xl active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center text-sm sm:text-base shadow-sm">
                      <MessageCircle size={18} className="mr-2"/> AVISAR SALIDA
                    </button>
                  ) : (
                    <div className="flex-1 bg-stone-200 text-stone-400 font-bold p-4 rounded-xl flex items-center justify-center text-xs text-center">Sin teléfono</div>
                  )}
                </div>

                <div className="flex gap-2">
                   {pedidoActual.telefono && (
                     <button onClick={enviarAvisoLlegada} className="flex-1 bg-white border-2 border-[#25D366] text-[#25D366] font-black p-3 rounded-xl active:scale-95 transition-all text-xs flex flex-col items-center justify-center shadow-sm">
                       <span>ESTOY AFUERA</span>
                     </button>
                   )}
                   <button onClick={avisarAdmin} className="flex-1 bg-stone-100 text-stone-600 font-black p-3 rounded-xl active:scale-95 transition-all text-xs flex flex-col items-center justify-center border border-stone-200 shadow-sm">
            ...
            {/* COLUMNA DERECHA (CON SCROLL): Check-out de productos */}
            <div className="flex-1 flex flex-col overflow-hidden md:border-l md:border-stone-100 md:pl-8">
              <h3 className="text-[11px] font-black text-red-500 uppercase tracking-widest mb-3 flex items-center gap-1 shrink-0"><Package size={14}/> Entregar estos items:</h3>
              <div className="flex-1 overflow-y-auto space-y-2 no-scrollbar pb-4 md:pb-2">
                {pedidoActual.items.map((it, i) => (
                  <label key={i} className={`flex items-start p-4 rounded-2xl border transition-all cursor-pointer ${it.sacado ? 'bg-red-50 border-red-200 opacity-60' : 'bg-white border-stone-200 shadow-sm hover:border-red-100'}`}>
          ...
          // VISTA ESTÁNDAR PARA MODO CARGA (VERTICAL Y HORIZONTAL IGUALES)
          <div className="flex flex-col flex-1 overflow-hidden">
             <div className="flex justify-between items-start mb-5 shrink-0">
              <div className="flex-1 pr-4">
                ...
                <p className="text-sm font-bold text-stone-500 mt-2 flex items-start"><MapPin size={14} className="mr-1.5 mt-0.5 text-red-500 shrink-0"/> <span className="leading-snug">{pedidoActual.direccion}</span></p>
              </div>
              <BotonRayo />
            </div>
            
            <h3 className="text-[11px] font-black text-emerald-500 uppercase tracking-widest mb-3 flex items-center gap-1 shrink-0"><Package size={14}/> Verificar en cocina:</h3>
            <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pb-4">
              {pedidoActual.items.map((it, i) => (
                <label key={i} className={`flex items-start p-4 rounded-2xl border transition-all cursor-pointer ${it.marcado ? 'bg-emerald-50 border-emerald-300 shadow-inner' : 'bg-white border-stone-200 shadow-sm hover:border-emerald-100'}`}>
        ...
        {/* Botones de navegación inferiores (fijos) */}
        <div className="pt-5 mt-auto flex justify-between gap-3 border-t-2 border-stone-100 bg-white shrink-0">
          <button onClick={() => setIndex(i => Math.max(0, i - 1))} disabled={index === 0} className="w-16 h-16 flex items-center justify-center bg-stone-100 text-stone-500 rounded-[1.5rem] active:scale-95 disabled:opacity-30 transition-transform shrink-0 shadow-inner"><ChevronLeft size={32} /></button>
          ...
          {modo === 'carga' ? (
            <button onClick={index === pedidos.length - 1 ? cambiarAModoReparto : () => setIndex(i => i + 1)} disabled={!pedidoActual.items.every(it => it.marcado)} className="flex-1 h-16 rounded-[1.5rem] font-black text-xl text-white bg-red-600 active:scale-95 transition-transform disabled:opacity-50 disabled:bg-stone-300 disabled:text-stone-500 shadow-lg shadow-red-200">
              {index === pedidos.length - 1 ? 'TERMINAR CARGA' : 'SIGUIENTE'}
            </button>
          ) : (
            <button onClick={index === pedidos.length - 1 ? finalizarRuta : () => { setIndex(i => i + 1); setEta(""); }} disabled={!pedidoActual.items.every(it => it.sacado)} className="flex-1 h-16 bg-stone-900 text-white rounded-[1.5rem] font-black text-xl active:scale-95 transition-transform disabled:opacity-50 shadow-lg shadow-stone-300">
              {index === pedidos.length - 1 ? 'TERMINAR RUTA' : 'ENTREGADO'}
            </button>
          )}
        </div>
      </div>
